...
export const Checkbox: React.FC<CheckboxProps> = ({ state = 'unchecked', onPress, style, accessibilityLabel }) => {
  ...
  return (
    <Pressable
      ...
      onPress={onPress}
      disabled={isDisabled}
      style={[styles.base, { borderColor: colors.outline }, checked && { backgroundColor: colors.primary, borderColor: colors.primary }, isDisabled && { opacity: 0.5 }, style]}
    >
      {checked ? <Ionicons name={state === 'indeterminate' ? 'remove' : 'checkmark'} size={14} color={colors.onPrimary} /> : null}
    </Pressable>
  );
};
